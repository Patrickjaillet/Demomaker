#version 330
uniform float iTime; uniform vec2 iResolution; uniform float iKick;
uniform float iBass;
uniform float iMid;
uniform float iHigh;
uniform float iBeat;
...
uniform sampler2D iWaveform;  // unit 9 — waveform  [0,1] uniform float iSceneProgress;
uniform sampler2D iChannel0; uniform sampler2D iChannel1;
out vec4 fragColor;
void main(){
    vec2 uv=gl_FragCoord.xy/iResolution.xy;
    vec2 px=1.0/iResolution.xy;
    float r=2.0+iKick*3.0;
    vec3 col=vec3(0.0); float w=0.0;
    for(int i=-8;i<=8;i++){
        float wt=exp(-float(i*i)/(2.0*r*r));
        col+=texture(iChannel1,uv+vec2(0.0,float(i)*px.y*2.0)).rgb*wt; w+=wt;
    }
